...
import 'package:SnipSnap/vendor/screens/addExpertScreen.dart';
          ...
          bottomNavigationBar: _isDataLoaded
              ? Container(
                  ...
                  child: TextButton(
                    ...
                    child: Text(
                      L10n.of(context).btn_add_new_expert,
                    ),
                  ),
                  decoration: BoxDecoration(
    ...
    try {
      showDialog(
          ...
          builder: (_) {
            return AlertDialog(
              title: Text(
                L10n.of(context).lbl_delete_expert,
              ),
              content: Text(
                  L10n.of(context).txt_confirmation_message_for_delete_expert),
              actions: [
                TextButton(
                  child: Text(
                    L10n.of(context).lbl_no,
                  ),
                  onPressed: () => Navigator.of(context).pop(),
                ),
                TextButton(
                  child: Text(
                    L10n.of(context).lbl_yes,
                  ),
                  onPressed: () async {
                    Navigator.of(context).pop();
                    await _deleteExpert(staffId as int, _index as int);
                  },
                )
              ],
            );
    ...
    try {
      ...
      if (isConnected) {
        await apiHelper.getExpertReview(staffId).then((result) {
          if (result != null) {
            if (result.status == "1") {
              // _expertList[_index].review = result.recordList as List<Review>;
            } else {
    ...
    try {
      ...
      if (isConnected) {
        await apiHelper.getExperts(global.user.id).then((result) {
          if (result != null) {
            if (result.status == "1") {
              List<Expert> _tList = result.recordList as List<Expert>;
